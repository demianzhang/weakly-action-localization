...
if __name__ == '__main__':
    ...
    if args.train:
        ...
        for data in dataloaders['train']:
            ...
            # in_answer = np.zeros((len(other[2]), 1))
            # start_answer = []
            # fp_frame = []

            # sort_start = []
            # for i in pos[0]:
            #     sort_start.append([int(line[i]),cam[i]])
            # sort_start.sort(key=lambda x:x[0])
            # res=[]
            # for i in range(len(sort_start)):
            #     if i==0:
            ...
            #         segment += 1
            #     else:
            #         res.append([start,end,round(score/segment,6)])
            #         segment = 1
            #         start=sort_start[i][0]
            #         end=sort_start[i][0]+48
            #         score = sort_start[i][1]
            ...
            # # for item in res:
            # #     if item:
            # #         ftmp.write("{} {} {:.3f}".format(item[0],item[1],item[2]))
            # #     ftmp.write("\n")
            # for part in res:
            #     if part[1] - part[0]<30:
            #         continue
            #     flag=0
            #     for item in other[2]:
            #         if temporal_iou([part[0],part[1]],[item[1].numpy()[0],item[2].numpy()[0]])>=0.5:
            #             right+=1
            #             flag=1
            #             break
            #     if flag==0:
            #         fp+=1
            #         #print("fp st:{} ed:{} during:{},score:{}".format(round(part[0]/30,1),round(part[1]/30,1),round((part[1]-part[0]) / 30,1),part[2]))
            # all+=len(other[2])
            # print("right:{},fp:{},all:{},acc:{}".format(right,fp,all,right/all))
            # print("video:{}".format(other[0]))
            ...
            #     if i == 0:
            #         start = sort_start[i][0]
            #         end = sort_start[i][0] + 50
            #         score = sort_start[i][1]
            #         continue
            #     if sort_start[i][0] < end + 18:
            #         end = sort_start[i][0] + 50
            #         score = max(score, sort_start[i][1])
            #     else:
            #         res.append([start, end, round(score, 6)])
            ...
            #     number=0
            #     for i in range(400):
            #         if int(line[i]) > item[1]:
            #             result.append([item[0],item[1],tmp/number])
            #             break
            #         if int(line[i]) >= item[0]:
            #             tmp+=(float(val_rgb_cam[num_step][i])+cam[i])/2
            #             number+=1
            #
            # num_step += 1
            # result = np.array(res)
            # result = temporal_nms(result,0.3)
            #
            #
            # for part in result:
            #     if part[1] - part[0] < 30:
            ...
            #     flag=0
            #     for item in other[2]:
            #         if int(line[i]) > item[1].numpy()[0]-30 and int(line[i]) < item[2].numpy()[0]+30:
            #             in_answer[j]+=1
            #             start_answer[j].append(int(line[i]))
            #             flag=1
            #             break
            #         j+=1
            #     if flag==0:
            #         fp+=1
            #         fp_frame.append(int(line[i]))
            # j=0
            # for i in other[2]:
            #     print("start:{},end:{},num{}".format(i[1].numpy()[0]/30,i[2].numpy()[0]/30,in_answer[j]))
            #     for item in start_answer[j]:
            #         print(int(item/30),end=" ")
            #     print()
            #     j+=1
            # print("video:{},fp:{}".format(other[0],fp))
            # fp_frame.sort()
            # # for item in fp_frame:
            # #     print(item/30)
            #
            # # check label
            # predict = outputs.data.cpu().numpy()
            # labels=labels.numpy()
            # if(predict.argmax()==labels.squeeze()):
            #     continue
            # print('x' * 20)
